...
// Copyright (c) 2016-2017 The ADD developers
...
// Add: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_19_CHANGE_COLLATERAL) r = SPORK_19_CHANGE_COLLATERAL_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_19_CHANGE_COLLATERAL") return SPORK_19_CHANGE_COLLATERAL;
    return -1;
}
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_19_CHANGE_COLLATERAL) return "SPORK_19_CHANGE_COLLATERAL";
