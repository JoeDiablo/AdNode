...
#include "ui_zaddcontroldialog.h"
...
std::list<std::string> ZAddControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAddControlDialog::listMints;

ZAddControlDialog::ZAddControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAddControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAddControlDialog::~ZAddControlDialog()
{
    delete ui;
}

void ZAddControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAddControlDialog::updateList()
{
...
void ZAddControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAddControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAdd_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAddControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAddControlDialog::GetSelectedMints()
{
...
void ZAddControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
