...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Add address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no Add: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert add:// to add:
    //
    //    Cannot handle this later, because add:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open add.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Add.lnk";
}
...
{
    return GetAutostartDir() / "add.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a add.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the add app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add add app to startup item list
