                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zADD ought to be enough for anybody." - Bill Gates, 2017
    ui->zADDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzADDSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzADD->setEnabled(false);
        ui->pushButtonMintzADD->setToolTip(tr("zADD is currently disabled due to maintenance."));

        ui->pushButtonSpendzADD->setEnabled(false);
        ui->pushButtonSpendzADD->setToolTip(tr("zADD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zADDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzADD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zADD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zADD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zADD in ") +
...
void PrivacyDialog::on_pushButtonSpendzADD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zADD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zADD
        sendzADD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zADD
    sendzADD();
}

void PrivacyDialog::on_pushButtonZAddControl_clicked()
{
    ZAddControlDialog* zAddControl = new ZAddControlDialog(this);
    zAddControl->setModel(walletModel);
    zAddControl->exec();
}

void PrivacyDialog::setZAddControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAddSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzADD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Add Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zADDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zADD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ADD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zADD</b>";
    ...
    // use mints from zAdd selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAddControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAddControlDialog::GetSelectedMints();
    }
    ...
    // Clear zadd selector in case it was used
    ZAddControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAdd Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Add, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAdd Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zADD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zADD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zADD "));
    ui->labelzADDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzADDSyncStatus->setVisible(fShow);
}
